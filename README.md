...
| `sequentialTests` | `true` | Run at most one test JVM at a time across all modules — mutual exclusion, not ordering. Set to `false` to let test JVMs run in parallel across modules. |
