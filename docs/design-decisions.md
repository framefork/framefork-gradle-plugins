...
## 13. Sequential tests (opt-out) — a one-permit shared service, not a `mustRunAfter` chain

`framefork { sequentialTests }` (default **on**, opt out with `sequentialTests = false`). When on, every `Test` task across the whole consumer build runs mutually exclusively — at most one test JVM at a time — while everything else stays parallel. The mechanism is a no-op shared `BuildService` (`TestSerializerService`) registered with `maxParallelUsages = 1` and declared as `usesService(...)` on every `Test` task: a one-permit semaphore the Gradle scheduler enforces against whatever actually runs.

- **Why on by default**: every framefork repo wants it. The modules carry heavy test JVMs — testcontainers, forked `jcmd` probes, database fixtures — that already saturate a machine on their own, so cross-module test parallelism buys little but courts flaky resource contention and OOMs. The service serializes only `Test` tasks; every non-test task (compilation, static analysis, packaging) stays fully parallel, so the wall-clock cost is small. A repo that genuinely benefits from parallel test JVMs opts out with `sequentialTests = false`.
...
- **Kotlin `@AutoService` needs a Kotlin-native backend — wired reactively, never auto-applied.** javac's `annotationProcessor` only ever processes Java sources, so `@AutoService` on a *Kotlin* class generates nothing through it and **fails open** (a silent no-op) — the worst failure mode for a registration mechanism. Kotlin annotation processing runs through KSP or kapt, so the plugin reacts (`pluginManager.withPlugin`, order-independent) to whichever the module applied: **KSP** gets `dev.zacsweers.autoservice:auto-service-ksp` (google/auto ships only a javac/kapt processor and declined KSP support, so this is the third-party KSP2-compatible reimplementation — with the caveat that, unlike the javac processor, it does **not** merge hand-written `META-INF/services` files); **kapt** gets Google's own `auto-service` on the `kapt` configuration (kapt suppresses javac's `annotationProcessor` by default, so there's no double-processing — and `keepJavacAnnotationProcessors = true` must never be suggested, as it re-runs the processor over the Java sources and duplicates entries). KSP is documented as preferred: kapt is in maintenance mode.
  - **Why not auto-apply a backend.** The dependency *wiring* is reactive and needs no deferral, but the plugin deliberately does not *apply* KSP or kapt itself. It can't know a KSP plugin version compatible with the consumer's Kotlin (KSP versions are pinned `<kotlin>-<ksp>` pairs and, unlike the `kotlin(...)` companions, KSP is not on the suite's classpath), and silently applying maintenance-mode kapt would be the wrong default. So instead of guessing, a Kotlin module that wires neither backend gets a **warning** (build still succeeds) naming both recipes — better a diagnosable warning than a silent registration gap.
  - **The warning is the one place this feature uses the sanctioned `afterEvaluate`.** Absence of a backend can't be detected reactively (you can't react to a plugin that is *never* applied), and KSP/kapt may be applied *after* `auto-service` in the consumer's `plugins {}` block, so an eager check would false-warn. The check is deferred to `afterEvaluate` — the same sanctioned exception `configureStagingPublishing` documents (§9) — which stays CC-safe because it only reads plugin state and logs. The dependency wiring itself uses no `afterEvaluate`.
